...
private:
  map<string, Vertex *> vertexes_;
  bool directionalEdges_;

  // find vertex given label. return nullptr if not found
  Vertex *findVertex(const string &label) const;

  // find vertex given label. Make a new vertex if not found
  Vertex *findMakeVertex(const string &label);

  // helper function for function with the same name
  bool connect(const string &from, const string &to, int weight, bool cycle);
  bool disconnect(const string &from, const string &to, bool cycle);

  // comparator for sort verter of edges in given vertex
  static bool edgesSortByEndVertexAsn(Edge *e1, Edge *e2);
  static bool edgesSortByEndVertexDes(Edge *e1, Edge *e2);

  // Kruskal helper functions
  // find parents from map recursively.
  Vertex *find(map<Vertex *, Vertex *> &parents, Vertex *v) const;
  // recursively find parents for v1 and v2.
  bool isConnected(map<Vertex *, Vertex *> &parents, Vertex *v1,
                   Vertex *v2) const;
};
